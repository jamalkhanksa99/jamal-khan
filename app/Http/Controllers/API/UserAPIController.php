        ...
        try {
            ...
            if (auth()->attempt(['email' => $request->input('email'), 'password' => $request->input('password')])) {
                ...
                if ($user->eProviders()->exists()) {
                    $provider = $user->eProviders[0];

                    if (!$provider->accepted)
                        return $this->sendError(__('auth.provider_not_accepted'));

                    $providerType = $provider->eProviderType;
                    $user->user_type = ['id' => $providerType->id, 'name' => $providerType->name];
                } else {
                    $user->user_type = ['id' => 1, 'name' => 'Customer'];
                }
